...
import tomd
import requests
csdn_id = "jiacong_wang"
targetId="105593209"
parser = argparse.ArgumentParser()
# 修改成CSDN的usename即可
parser.add_argument('-i', '--id', type=str,
                    default='default', help='weixin_43486940')
...
def request_blog_column(id):
    ...
    for span in spans:
        ...
        blogs_num = re.findall(
            r'<a class="clearfix special-column-name" target="_blank" href=\"' +
            href + '\".*?<span class="special-column-num">(.+?)篇</span>',
...
def append_blog_info(blog_column_url, blog_column_name, blogs):
    ...
    blogs_list = blog_span.find_all(
        'ul', attrs={'class': 'column_article_list'})
    ...
    for arch_blog_info in blogs_list:
        ...
        for blog_info in blogs_list:
            ...
            blog_title = blog_info.find('h2', attrs={'class': "title"}).get_text(
            ).strip().replace(" ", "_").replace('/', '_')
            blog_dict = {'column': blog_column_name,
